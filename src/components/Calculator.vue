<template>
	<div class="calculator border p-4 rounded bg-light shadow-lg mb-4">
		...
		<!-- Results Display Div -->
		<div
			v-if="calculatedValue"
			class="results d-flex flex-column align-items-center text-center"
		>
			<h1 class="mb-3">You Need:</h1>
			<h5>
				<span class="text-danger">{{ calculatedValue.toFixed(2) }}</span> lbs of
				vermicompost
			</h5>
			<h5 class="text-center">or</h5>
			<h5>
				<span class="text-danger">{{ (calculatedValue / 2.2).toFixed(2) }}</span> kg
				of vermicompost
			</h5>
			<h5 class="text-center">or</h5>
			<h5>
				<span class="text-danger">{{
					calculatedValue | calculatedNumberOfBags
				}}</span>
				x 4.5 lbs bags of vermicompost
			</h5>
			<b-button type="reset" variant="danger" @click="calculatedValue = ''"
				>Reset</b-button
			>
		</div>
	</div>
...
export default {
	...
	filters: {
		calculatedNumberOfBags(calculatedValue) {
			return calculatedValue < 4.5
				? 1
				: Math.ceil((calculatedValue / 4.5).toFixed(1));
		},
	},
};
</script>
